	...
	int outputOrderType)
{
	...
	if (isReadFile == 1) {
		...
		printRunningTime_and_Comparison(comparisonsCount, runningTime, outputOrderType);
	}
	...
	// COMMAND 2 AND 3:
		...
		if (dataType == -1) {
			for (int i = 0; i < 4; i++) {
				...
				else if (i == 1) writeToFile("input_2.txt", a, dataSize);
				else if (i == 2) writeToFile("input_3.txt", a, dataSize);
				else if (i == 3) writeToFile("input_4.txt", a, dataSize);
...
void AlgorithmMode(string algorithm, string input, string orderData, string outputOrder, long long& comparisonsCount, float& runningTime) {
	int dataSize = -1;
	int dataType = 0;
